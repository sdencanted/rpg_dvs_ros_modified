...
// DvxplorerRosDriver::DvxplorerRosDriver(const rclcpp::NodeOptions & options,const  rclcpp::NodeOptions & options_private) :
...
DvxplorerRosDriver::DvxplorerRosDriver(const std::string & node_name) :
    ...
    event_struct_pub_ = create_publisher<dvs_msgs::msg::EventStruct>(ns+ "/eventStruct",10);
	...
	=create_subscription<std_msgs::msg::Empty>((ns + "/calibrate_imu").c_str(), 1, std::bind(&DvxplorerRosDriver::imuCalibrationCallback,this,std::placeholders::_1));
	...
	if (master_) {
		...
		reset_sub_
		=create_subscription<builtin_interfaces::msg::Time>((ns + "/reset_timestamps").c_str(), 1, std::bind(&DvxplorerRosDriver::resetTimestampsCallback,this,std::placeholders::_1));
	...
	param_callback_handle_=add_on_set_parameters_callback(
        std::bind(&DvxplorerRosDriver::callback, this, std::placeholders::_1));
	...
	if (reset_timestamps_delay > 0.0) {
		...
		timestamp_reset_timer_
			// =create_wall_timer<int64_t,std::milli,std::function<void()>>(rclcpp::Duration(reset_timestamps_delay), std::bind(&DvxplorerRosDriver::resetTimerCallback,this), my_callback_group);
			=create_wall_timer(std::chrono::nanoseconds(long(reset_timestamps_delay*1e9)), [this]() -> void { resetTimerCallback(); });
			// = nh_.createTimer(rclcpp::Duration(reset_timestamps_delay), &DvxplorerRosDriver::resetTimerCallback, this);
		RCLCPP_INFO(this->get_logger(),"Started timer to reset timestamps on master DVS for synchronization (delay=%3.2fs).",
...
void DvxplorerRosDriver::updateImuBias() {
	...
	RCLCPP_INFO(this->get_logger(),"Acceleration biases: %1.5f %1.5f %1.5f [m/s^2]", bias.linear_acceleration.x, bias.linear_acceleration.y,
		bias.linear_acceleration.z);
	RCLCPP_INFO(this->get_logger(),"Gyroscope biases: %1.5f %1.5f %1.5f [rad/s]", bias.angular_velocity.x, bias.angular_velocity.y,
...
void DvxplorerRosDriver::readout() {
    ...
    // dvs_msgs::msg::EventStruct::Ptr event_struct_msg;
	// dvs_msgs::msg::EventImage::Ptr event_image_msg;
    dvs_msgs::msg::EventStruct::UniquePtr event_struct_msg;
	...
	event_struct_msg = dvs_msgs::msg::EventStruct::UniquePtr(new dvs_msgs::msg::EventStruct());
				...
				if (type == POLARITY_EVENT) {
					...
					#pragma omp parallel num_threads(4) // 4 threads is the minimum number of threads to achieve 100hz
					{
						...
						for(int i=0; i<omp_get_num_threads(); i++) {
							#pragma omp ordered
							event_struct_msg->event_arr.data.insert(event_struct_msg->event_arr.data.end(), eventArr_local.begin(), eventArr_local.end());
							event_struct_msg->event_time.data.insert(event_struct_msg->event_time.data.end(), eventTime_local.begin(), eventTime_local.end());
						}
					...
					event_image_msg->size = event_struct_msg->event_time.data.size();

                    // throttle event messages
                    if (count_R == reached)
                    {
						// RCLCPP_INFO(this->get_logger(),"published");
                        event_struct_pub_->publish(std::move(event_struct_msg));
