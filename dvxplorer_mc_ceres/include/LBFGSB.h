...
namespace LBFGSpp
{
    ...
    public:
        ...
        inline int minimize(Foo &f, Vector &x, Scalar &fx, const Vector &lb, const Vector &ub, uint64_t us = 0)
        {
            using std::abs;
            ...
            for (;;)
            {
                ...
                if (us > 0)
                {
                    const auto current_clock_linesearch = std::chrono::system_clock::now();
                    LineSearch<Scalar>::LineSearch(f, m_param, m_xp, m_drt, step_max, step, fx, m_grad, dg, x, us - std::chrono::duration_cast<std::chrono::microseconds>(current_clock_linesearch - start_clock).count());
                }
                else
                {
                    LineSearch<Scalar>::LineSearch(f, m_param, m_xp, m_drt, step_max, step, fx, m_grad, dg, x, us);
                }
